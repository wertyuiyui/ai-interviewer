...
- Agent：`/root`。
- 状态：`completed`。
- 摘要：所有简历入口继续共用 `ResumeParser.parse()`；正常完整识别仍只请求一次。百炼请求失败、结构化结果无效，或原文存在明确教育/实习/项目/技能章节而首轮整章漏识别时，最多补一次请求；补识别只填首轮空缺章节，不覆盖首轮已有内容。补请求失败时保留首轮有效结果，避免一次瞬时故障把可用简历整体判失败。
...
- 风险或后续事项：重试只针对请求异常和明确章节整段缺失，避免每份简历固定双倍调用；无章节标题的自由排版仍以首轮模型和既有保守原文恢复为准。生产发布后需使用服务器既有配置做脱敏样例探测，并复核三处健康端点。
